...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # ================================================================ #
        b1_matrix = np.matmul(np.array([np.ones(len(X))]).transpose(), np.array([b1]))
        b2_matrix = np.matmul(np.array([np.ones(len(X))]).transpose(), np.array([b2]))
        
        h1 = np.matmul(X, W1.transpose()) + b1_matrix
        a2 = np.zeros(h1.shape)
        a2 = (h1 > 0).astype(int) * h1 # activation with input of h1
        h2 = np.matmul(a2, W2.transpose()) + b2_matrix
        scores = h2
        ...
        def softmax_loss(x, y):
            ...
            # ================================================================ #
            y_target_matrix = X.shape[0]
            x -= x.max() 
            x = np.exp(x)
            scores_sums = np.sum(x, axis=1)
            scores_2 = x[range(X.shape[0]), y]
            loss = scores_2 / scores_sums
            loss = -np.sum(np.log(loss)) / X.shape[0]
            dx = np.divide(x, scores_sums.reshape(X.shape[0], 1))
            dx[range(X.shape[0]), y] = - (scores_sums - scores_2) / scores_sums
            dx /= X.shape[0]
        ...
        def MSE_loss(y_pred, y_target):
            ...
            y_target_matrix = np.zeros(list(y_pred.shape))
            
            for i in range(len(y_target)):
                y_target_matrix[i][y_target[i]] = 1  #set all values to 1

            loss = (np.linalg.norm(y_pred - y_target_matrix) ** 2) / (2 * len(X))
            dy_pred = (y_pred - y_target_matrix) / N
        ...
        # ================================================================ #
        reg_loss = reg / 2 * (np.linalg.norm(W1) ** 2 + np.linalg.norm(W2) ** 2)
